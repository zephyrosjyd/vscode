...
if [[ "$OSTYPE" == "darwin"* ]]; then
	...
	LINUX_NO_SANDBOX="--no-sandbox --disable-gpu --disable-gpu-compositing" # Electron 6 introduces a chrome-sandbox that requires root to run. This can fail. Disable sandbox via --no-sandbox.
fi
