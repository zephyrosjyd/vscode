...
import { ExtensionDownloadsCache } from 'vs/platform/extensionManagement/node/extensionDownloader';
...
export class ExtensionManagementService extends Disposable implements IExtensionManagementService {
	...
	private readonly extensionDownloadsCache: ExtensionDownloadsCache;
	...
	constructor(
		...
		this.extensionDownloadsCache = this._register(instantiationService.createInstance(ExtensionDownloadsCache));
			...
			try {
				...
				this.downloadInstallableExtension(extension, operation)
					.then(installableExtension => this.installExtension(installableExtension, ExtensionType.User, cancellationToken)
						.then(local => this.extensionDownloadsCache.delete(URI.file(installableExtension.zipPath)).finally(() => { }).then(() => local)))
	...
	private downloadInstallableExtension(extension: IGalleryExtension, operation: InstallOperation): Promise<InstallableExtension> {
		...
		return this.extensionDownloadsCache.downloadExtension(extension, operation)
			.then(
				zip => {
